...
/* Header */
.magnera-header {
    background: linear-gradient(135deg, #13153a 0%, #0d0f1f 100%);
...
.magnera-logo {
    background: linear-gradient(135deg, #ec4899, #8b5cf6, #6d28d9);
    padding: 10px 14px;
    border-radius: 10px;
    font-size: 22px;
    font-weight: 900;
...
.stButton > button:hover {
    ...
    box-shadow: 0 0 30px rgba(139,92,246,0.55) !important;
}
...
# ─────────────────────────────────────────────
# OPTIMIZER LOGIC — MINERAÇÃO + SCIP SOLVER
# ─────────────────────────────────────────────
def run_optimization(orders, master_width, max_knives, kert, linear_meters, grammage,
                     max_setups=5, otif_meta=98.0,
                     l_min_fator=0.94, max_larg_esquema=3, diff_limit=50.0,
                     premio_avanco=1.0, l1_tiradas=0.5, l2_setups=500.0,
                     l3_over=8.0, custo_falta=50.0, min_runs_setup=1,
                     otif_max_fator=1.10, folga_fator=0.05):
    # ── Parâmetros ──────────────────────────────────────────────────────────
    LU_NOMINAL   = float(master_width)
    LU_MAX       = LU_NOMINAL
    MAX_FACAS    = int(max_knives)
    MAX_SETUPS   = int(max_setups)
    METRAGEM     = float(linear_meters)
    G            = float(grammage)
    KERT         = float(kert)
    L_MIN_FATOR  = float(l_min_fator)
    MAX_LARG_ESQUEMA = int(max_larg_esquema)
    DIFF_LIMIT   = float(diff_limit)
    PREMIO_AVANCO = float(premio_avanco)
    L1_TIRADAS   = float(l1_tiradas)
    L2_SETUPS    = float(l2_setups)
    L3_OVER      = float(l3_over)
    CUSTO_FALTA  = float(custo_falta)
    MIN_RUNS_SETUP = int(min_runs_setup)
    OTIF_MAX     = float(otif_max_fator)
    FOLGA        = float(folga_fator)

    # Mapeia larguras brutas únicas
    seen = []
    arruelas_map = {}
    net_map = {}
    for o in orders:
        bw = int(o["width"]) + int(o["arruela"])
        if bw not in seen:
            seen.append(bw)
            arruelas_map[bw] = int(o["arruela"])
            net_map[bw]      = int(o["width"])

    larguras = seen
    larguras_efetivas = [w + KERT for w in larguras]

    def rolos_demanda(bw):
        kgr = (bw / 1000) * METRAGEM * (G / 1000)
        total_kg = sum(o["weightKg"] for o in orders
                       if int(o["width"]) + int(o["arruela"]) == bw)
        return math.ceil(total_kg / kgr) if kgr > 0 else 0

    demandas_rolos = [rolos_demanda(w) for w in larguras]
    ...
    def minerar_pool():
        ...
        def varredura(idx, qtd, l_acum, vetor):
            if idx == len(larguras):
                if l_acum >= L_MIN_BRUTA and l_acum <= LU_MAX:
                    pool.append({"vetor": list(vetor), "l_real": l_acum})
                return
            max_q = min(MAX_FACAS - qtd,
                        int((LU_MAX - l_acum) // larguras_efetivas[idx]))
            for q in range(max_q + 1):
                vetor[idx] = q
                varredura(idx + 1, qtd + q,
                          l_acum + q * larguras_efetivas[idx], vetor)
                vetor[idx] = 0
        ...
        for p in pool:
            ...
            p["tipo"] = ("Mono" if len(usadas) == 1
                         else "Duo" if len(usadas) == 2 else "Trio")
            final_pool.append(p)
        return final_pool
    ...
    if not pool_detalhado:
        return [], "Nenhum esquema válido encontrado. Verifique as larguras e restrições."

    if not ORTOOLS_OK:
        return [], "OR-Tools não instalado. Execute: pip install ortools"

    # ── ETAPA 4: SOLVER SCIP ────────────────────────────────────────────────
    solver = pywraplp.Solver.CreateSolver("SCIP")
    if solver is None:
        return [], "Solver SCIP indisponível."
    ...
    total_prod_kg_runs = sum(
        x[j] * ((pool_detalhado[j]["l_real"] * METRAGEM * G) / 1e6)
        for j in range(NJ))
    peso_dem_rolos = sum(
        demandas_rolos[i] * ((larguras[i] * METRAGEM * G) / 1e6)
        for i in range(NI))
    ...
    peso_falta_kg = sum(
        underfill[i] * ((larguras[i] * METRAGEM * G) / 1e6)
        for i in range(NI))
    ...
    if status not in (pywraplp.Solver.OPTIMAL, pywraplp.Solver.FEASIBLE):
        return [], "Solver não encontrou solução. Tente relaxar restrições (max setups, OTIF, etc.)."

    # ── Converte resultado para cards de exibição ───────────────────────────
    combinations = []
    combo_id = 1

    for j in range(NJ):
        tiradas_j = int(round(x[j].solution_value()))
        if tiradas_j <= 0:
            continue
        ...
        for i, bw in enumerate(larguras):
            if vetor[i] > 0:
                widths_in.append(bw)
                net_widths_in.append(net_map[bw])
                arruelas_in.append(arruelas_map[bw])
                roll_counts.append(int(vetor[i]))

        waste_mm  = round(LU_NOMINAL - p["l_real"], 2)
        waste_pct = round(p["slu"], 4)

        kg_per_tirada = sum(
            roll_counts[k] * (widths_in[k] / 1000) * METRAGEM * (G / 1000)
            for k in range(len(widths_in)))
        m2_per_tirada = sum(
            roll_counts[k] * (widths_in[k] / 1000) * METRAGEM
            for k in range(len(widths_in)))
        ...
        combinations.append({
            ...
            "tipo":          p["tipo"],
            "widths":        widths_in,
            "netWidths":     net_widths_in,
            "arruelas":      arruelas_in,
            "rollCounts":    roll_counts,
            "waste":         waste_mm,
            "wastePercentage": waste_pct,
            "tiradas":       tiradas_j,
            "totalKg":       round(kg_per_tirada * tiradas_j, 2),
            "m2":            round(m2_per_tirada  * tiradas_j, 2),
        })
        combo_id += 1
...
if st.session_state.simulated and st.session_state.combinations:
    ...
    slu_color   = "#10b981" if weighted_slu < 1.0 else ("#7c3aed" if weighted_slu < 1.3 else "#ef4444")
    ...
    st.bar_chart(chart_data_indexed, color="#8b5cf6", height=220)
