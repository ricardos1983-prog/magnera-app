...
.kpi-grid-6 { display: grid; grid-template-columns: repeat(6, 1fr); gap: 12px; margin: 16px 0; }
.kpi-grid-3 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 16px 0; }
...
<div class="magnera-header">
  <img src="data:image/png;base64,{LOGO_B64}" style="height:48px;object-fit:contain;" />
...
# UPLOAD
# ─────────────────────────────────────────────
st.markdown('<div class="section-title">📂 Arquivo de Entrada</div>', unsafe_allow_html=True)
uploaded = st.file_uploader(
    "Selecione o arquivo Excel (.xlsx) com as abas: config | Pedidos | Matriz_LU | Matriz_Arruelas",
    type=["xlsx"],
...
if uploaded:
    try:
        df_config  = pd.read_excel(uploaded, sheet_name="config")
        df_pedidos = pd.read_excel(uploaded, sheet_name="Pedidos")
        df_lu      = pd.read_excel(uploaded, sheet_name="Matriz_LU")
        df_arr     = pd.read_excel(uploaded, sheet_name="Matriz_Arruelas")

        config_preview = dict(zip(df_config.iloc[:, 0].astype(str).str.strip(),
                                  df_config.iloc[:, 1]))
        M = str(config_preview.get("Maquina", "?")).strip()
        T = str(config_preview.get("Tecnologia", "?")).strip()
        S = str(config_preview.get("Surfactante", "?")).strip()
        C = str(config_preview.get("Calandra", "?")).strip()
        G = float(config_preview.get("Gramatura_GSM", 0))
        METRAGEM = float(config_preview.get("Metragem_mL", 0))

        col_p1, col_p2, col_p3, col_p4, col_p5, col_p6 = st.columns(6)
        col_p1.metric("Máquina",   M)
        col_p2.metric("Tecnologia", T)
        col_p3.metric("Surfactante", S)
        col_p4.metric("Calandra",  C)
        col_p5.metric("Gramatura", f"{G:g} g/m²")
        col_p6.metric("Metragem",  f"{METRAGEM:,.0f} m")

        st.markdown('<div class="orange-divider"></div>', unsafe_allow_html=True)

        # Pedidos preview
        st.markdown('<div class="section-title">📋 Demandas Detectadas</div>', unsafe_allow_html=True)
        col_larg = col_val = None
        for col in df_pedidos.columns:
            nc = str(col).lower()
            if "largura" in nc: col_larg = col
            elif any(x in nc for x in ["valor", "peso", "pedido", "demanda", "kg"]): col_val = col
        if not col_larg: col_larg = df_pedidos.columns[0]
        if not col_val:  col_val  = df_pedidos.columns[1]
        df_show = df_pedidos[[col_larg, col_val]].dropna()
        df_show.columns = ["Largura (mm)", "Demanda (kg)"]
        df_show = df_show[pd.to_numeric(df_show["Largura (mm)"], errors="coerce").notna()]
        st.dataframe(df_show.reset_index(drop=True), use_container_width=True, height=200)
        ...
        if st.button("⚡  OTIMIZAR ESQUEMA DE CORTE"):
            with st.spinner("⏳ Minerando esquemas e resolvendo ILP com SCIP..."):
                t0 = time.time()
                result, err = run_optimization(df_config, df_pedidos, df_lu, df_arr)
    ...
    st.markdown("""
    <div class="info-box">
    📌 <b>Formato esperado do Excel:</b><br>
    &nbsp;&nbsp;• <b>config</b> — parâmetros da campanha (Maquina, Gramatura_GSM, Meta_OTIF, etc.)<br>
    &nbsp;&nbsp;• <b>Pedidos</b> — colunas Largura_mm e Valor_Kg<br>
    &nbsp;&nbsp;• <b>Matriz_LU</b> — larguras úteis por máquina/gramatura<br>
    &nbsp;&nbsp;• <b>Matriz_Arruelas</b> — kerf por largura/máquina
    </div>
    """, unsafe_allow_html=True)
